      ...
      <ul className="flex gap-2">
        {pageNumbers.map((num, idx) => {
          if (typeof num === 'string') {
            return (
              <p
                className="self-end text-sm font-medium text-blue-700"
                key={num + idx}
              >
                ...
              </p>
            ...
            return (
              <button
                className={`flex h-[36px] items-center justify-center md:h-[40px] ${num === meta.page ? 'bg-blue-700 text-white hover:bg-blue-500' : 'border border-blue-700 bg-transparent text-blue-700 hover:border-blue-500 hover:bg-blue-500 hover:text-white'} aspect-square cursor-pointer rounded-[10px] p-2 text-xs font-medium transition-colors ease-out md:text-sm`}
